...
router.post('/:id/generate-explanation', optionalAuthenticate, async (req, res) => {
  try {
    ...
    //
    // Every failure path below used to be a bare 400 with nothing shown to
    // the student — "no explanation available" felt broken even when the
    // underlying reason was legitimate (missing data, an AI that couldn't
    // confidently solve it). Every path now instead returns 200 with an
    // honest, specific fallback message — never a fabricated answer, just
    // never a dead end either. None of these fallbacks are written to the
    // DB `explanation` column, so they stay retryable once the underlying
    // issue (missing options, an unconfirmed answer) is actually fixed.
    let autoSolvedExplanation = null;
    if (!correct_answers.length && question.question_type !== 'essay') {
      if (!options.length) {
        return res.json({
          explanation: 'This question is missing its answer options in the question bank, so an explanation can\u2019t be generated yet. Please ask your teacher to check this question in Question Bank.',
          cached: false, unverified: true,
        });
      }
      ...
      if (!solved.solvable || !solved.correct_answer_letter) {
        return res.json({
          explanation: 'The correct answer for this question hasn\u2019t been confirmed yet, so a full explanation isn\u2019t available. ' +
            (solved.reason ? `(${solved.reason}) ` : '') +
            'Your teacher can confirm the correct answer in Question Bank to unlock a full explanation.',
          cached: false, unverified: true,
        });
      }
      ...
      if (!solvedAnswerText) {
        return res.json({
          explanation: 'This question\u2019s answer options don\u2019t line up correctly in the question bank, so an explanation can\u2019t be generated yet. Please ask your teacher to check this question in Question Bank.',
          cached: false, unverified: true,
        });
      }
